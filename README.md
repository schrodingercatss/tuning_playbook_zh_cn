...
-   如果训练误差（或甚至验证误差）可以无限地改善，我们是否应该在我们能接受的情况下一直训练？答案是不必要。
    ...
    -   随着试训时间接近难以忍受，及时调整实验设计对于发布候选模型将变得更加重要，但我们之后可以完成的实验次数却越来越少(受到时间的限制)。
    -   仅训练约 10% 的生产长度时，我们可能可以回答很多问题，但始终存在这样的风险，即我们在这个时间限制下的结论不适用于生产长度的 20% 的实验，更不用说 100% 了.
-   在多轮训练中进行调优，并增加每轮训练的步数限制，这是一种明智的做法。
    -   我们可以想做多少轮就做多少轮，但通常 1-3 轮是最实用的。
    -   从本质上讲，应该尽量使用花费时间非常快的试验来获得对问题的理解，权衡单次调优的彻底性和单次调优对最终实验的相关性。
